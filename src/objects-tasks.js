...
function sellTickets(queue) {
  ...
  let canSell = true;
  queue.forEach((bill) => {
    if ((bill === 50 && billsOf25 < 1) || (bill === 100 && billsOf25 < 3)) {
      canSell = false;
    }
    billsOf25 += 1;
  });
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    return a.country === b.country
      ? a.city.charCodeAt() - b.city.charCodeAt()
      : a.country.charCodeAt() - b.country.charCodeAt();
  });
}
...
function group(array, keySelector, valueSelector) {
  return array.reduce((multimap, item) => {
    if (!multimap.has(keySelector(item))) multimap.set(keySelector(item), []);
    multimap.get(keySelector(item)).push(valueSelector(item));
    return multimap;
  }, new Map());
}
